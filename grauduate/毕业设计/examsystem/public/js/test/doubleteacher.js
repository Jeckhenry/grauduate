...
$(function () {
    ...
    $('.cha').click(function() {
        ...
        if (str === '修改') {
            ...
            var $_middle = $('#middle').val()
            var $_1 = parent.find('[name="weeks"]').text()
            var $_2 = parent.find('[name="teacing"]').text()
            var $_3 = parent.find('[name="article"]').text()
            var $_4 = parent.find('[name="methods"]').text()
            var $_5 = parent.find('[name="example"]').text()
            var $_6 = parent.find('[name="exercise"]').text()
            $.ajax({
                url: '/api/changedoubleteacher',
                method: 'post',
                async: false,
                data: {
                    middle: $_middle,
